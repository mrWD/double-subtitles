const BASKET_ICON = '<svg viewBox="0 0 24 24" width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M20.5001 6H3.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path> <path d="M18.8332 8.5L18.3732 15.3991C18.1962 18.054 18.1077 19.3815 17.2427 20.1907C16.3777 21 15.0473 21 12.3865 21H11.6132C8.95235 21 7.62195 21 6.75694 20.1907C5.89194 19.3815 5.80344 18.054 5.62644 15.3991L5.1665 8.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path> <path d="M9.5 11L10 16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path> <path d="M14.5 11L14 16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path> <path d="M6.5 6C6.55588 6 6.58382 6 6.60915 5.99936C7.43259 5.97849 8.15902 5.45491 8.43922 4.68032C8.44784 4.65649 8.45667 4.62999 8.47434 4.57697L8.57143 4.28571C8.65431 4.03708 8.69575 3.91276 8.75071 3.8072C8.97001 3.38607 9.37574 3.09364 9.84461 3.01877C9.96213 3 10.0932 3 10.3553 3H13.6447C13.9068 3 14.0379 3 14.1554 3.01877C14.6243 3.09364 15.03 3.38607 15.2493 3.8072C15.3043 3.91276 15.3457 4.03708 15.4286 4.28571L15.5257 4.57697C15.5433 4.62992 15.5522 4.65651 15.5608 4.68032C15.841 5.45491 16.5674 5.97849 17.3909 5.99936C17.4162 6 17.4441 6 17.5 6" stroke="currentColor" stroke-width="1.5"></path> </g></svg>';
const LINK_ICON = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14 7H16C18.7614 7 21 9.23858 21 12C21 14.7614 18.7614 17 16 17H14M10 7H8C5.23858 7 3 9.23858 3 12C3 14.7614 5.23858 17 8 17H10M8 12H16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>';

let allCards = [];

chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === 'sync' && changes.cards?.newValue) {
    allCards = changes.cards.newValue;
    loadSavedCards();
  }
});

// Add search input event listener
document.addEventListener('DOMContentLoaded', () => {
  const searchInput = document.querySelector('#searchCards');
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      filterCards(e.target.value);
    });
  }
});

function filterCards(searchTerm) {
  const filteredCards = allCards.filter(card =>
    card.text.toLowerCase().includes(searchTerm.toLowerCase()) ||
    card.translation.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const listWrapper = getEmptyCardList();
  toggleEmptyTextVisibility(!filteredCards.length);

  if (filteredCards.length) {
    filteredCards.forEach((card, index) => {
      listWrapper.appendChild(createCardElement(card, index, allCards));
    });
  }
}

function loadSavedCards() {
  chrome.storage.sync.get('cards', item => {
    allCards = item.cards || [];
    const listWrapper = getEmptyCardList();

    toggleEmptyTextVisibility(!allCards.length);

    if (allCards.length) {
      allCards.forEach((card, index) => {
        listWrapper.appendChild(createCardElement(card, index, allCards));
      });
    }
  });
}

function toggleEmptyTextVisibility(show = true) {
  const savedCardsModal = document.querySelector('#savedCards')
    ?.querySelector('#emptyCards');

  if (show) {
    savedCardsModal?.classList.add('visible');
  } else {
    savedCardsModal?.classList.remove('visible');
  }
}

function getEmptyCardList() {
  const savedCardsModal = document.querySelector('#savedCards');
  let listWrapper = savedCardsModal?.querySelector('ul');

  if (listWrapper) {
    listWrapper.innerHTML = '';
  } else {
    listWrapper = document.createElement('ul');
    savedCardsModal?.appendChild(listWrapper);
  }

  return listWrapper;
}

function createCardElement(card, index, cards) {
  const cardElement = document.createElement('li');

  cardElement.appendChild(createTextElement(card.text));
  cardElement.appendChild(createTextElement(card.translation));

  if (card.sourceUrl) {
    cardElement.appendChild(createSourceLink(card));
  }

  cardElement.appendChild(createDeleteButton(cards, index));

  return cardElement;
}

function createDeleteButton(cards, index) {
  const deleteButton = document.createElement('button');

  deleteButton.classList.add('btn', 'deleteButton');

  deleteButton.innerHTML = BASKET_ICON;

  deleteButton.addEventListener('click', (e) => {
    e.stopPropagation();

    chrome.storage.sync.set({
      cards: cards.filter((_, i) => i !== index),
    });
  });

  return deleteButton;
}

function createSourceLink(card) {
  const wrapper = document.createElement('span');
  wrapper.classList.add('sourceLink');

  const button = document.createElement('button');
  button.classList.add('btn', 'sourceLinkButton');
  button.innerHTML = LINK_ICON;

  const label = formatSourceLabel(card.sourceUrl, card.timestamp);
  button.title = label;

  const labelSpan = document.createElement('span');
  labelSpan.textContent = label;
  wrapper.appendChild(button);
  wrapper.appendChild(labelSpan);

  button.addEventListener('click', (e) => {
    e.stopPropagation();
    navigateToSource(card.sourceUrl, card.timestamp);
  });

  labelSpan.addEventListener('click', (e) => {
    e.stopPropagation();
    navigateToSource(card.sourceUrl, card.timestamp);
  });

  return wrapper;
}

function formatSourceLabel(sourceUrl, timestamp) {
  let label = '';

  try {
    const url = new URL(sourceUrl);
    label = url.hostname.replace('www.', '');
  } catch {
    label = sourceUrl;
  }

  if (timestamp != null && !isNaN(timestamp)) {
    const mins = Math.floor(timestamp / 60);
    const secs = Math.floor(timestamp % 60);
    label += ` @ ${mins}:${secs.toString().padStart(2, '0')}`;
  }

  return label;
}

function navigateToSource(sourceUrl, timestamp) {
  chrome.runtime.sendMessage({
    message: 'navigateToSource',
    payload: { sourceUrl, timestamp },
  });
}

function createTextElement(text) {
  const textElement = document.createElement('span');

  textElement.textContent = text;

  return textElement;
}
